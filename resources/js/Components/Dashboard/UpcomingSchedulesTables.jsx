...
const UpcomingSchedulesTable = ({ data, height, handleResize, formatDate }) => {
    return (
        <div className="w-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-md rounded-2xl shadow-lg border border-white/30 dark:border-gray-600/30 p-6 relative transition-all duration-300">
            <h3 className="text-lg font-medium mb-4 text-gray-800 dark:text-gray-100">Upcoming Schedules</h3>
            <div 
                    ...
                    <tbody className="bg-transparent divide-y divide-gray-200 dark:divide-gray-700">
                        {data.length > 0 ? (
                            data.map((schedule, index) => (
                                ...
                                >
                                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{formatDate(schedule.date)}</td>
                                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{schedule.employee?.name || 'N/A'}</td>
                                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{schedule.sub_section?.name || 'N/A'}</td>
                                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{schedule.man_power_request?.shift?.name || 'N/A'}</td>
                                </motion.tr>
                            ))
                        ) : (
                            <tr>
