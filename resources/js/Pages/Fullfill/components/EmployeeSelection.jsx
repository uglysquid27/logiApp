        ...
        <div className="bg-white shadow-md mb-6 p-4 rounded-lg">
            ...
            <div className="gap-4 grid grid-cols-1 md:grid-cols-2">
                {Array.from({ length: request.requested_amount }).map((_, index) => {
                    ...
                    return (
                        <div key={employeeId || `slot-${index}`} className={`flex justify-between items-center space-x-3 p-3 rounded-md ${isCurrentlyScheduled ? 'bg-green-50 border border-green-200' : 'bg-gray-100'
                            }`}>
                            <div className="flex items-center space-x-2">
                                <input
                                    ...
                                    checked={!isEmptySlot}
                                    readOnly
                                    className={`w-5 h-5 form-checkbox ${isCurrentlyScheduled ? 'text-green-600' : 'text-blue-600'
                                        }`}
                                />
                                <span>
                                    <strong>{isEmptySlot ? `Slot ${index + 1} Kosong` : employee?.name}</strong> ({employee?.nik || 'N/A'})
                                    {employee && (
                                        // Inside the employee display
                                        <div className="mt-1 text-gray-500 text-xs">
                                            ...
                                            <span> | Skor: {employee.total_score.toFixed(2)}</span>
                            ...
                            <button
                                ...
                                className={`text-sm ${isCurrentlyScheduled ? 'text-green-600 hover:text-green-800' : 'text-blue-600 hover:text-blue-800'
                                    }`}
                            >
