...
export default function Revision({ manpowerRequestData, subSections, shifts }) {
  // Initialize timeSlots based on existing data if provided, otherwise with defaults
  const initialTimeSlots = {};
  if (shifts && Array.isArray(shifts)) {
    shifts.forEach(shift => {
      // Find existing data for this shift
      const existingSlot = manpowerRequestData.time_slots ? manpowerRequestData.time_slots[shift.id] : null;

      initialTimeSlots[shift.id] = {
        id: existingSlot?.id || null, // Include ID if it's an existing request for update
        requested_amount: existingSlot?.requested_amount !== undefined ? existingSlot.requested_amount : '',
        male_count: existingSlot?.male_count !== undefined ? existingSlot.male_count : 0,
        female_count: existingSlot?.female_count !== undefined ? existingSlot.female_count : 0,
        start_time: existingSlot?.start_time || shift.start_time || '',
        end_time: existingSlot?.end_time || shift.end_time || '',
      };
    });
  }

  const { data, setData, put, processing, errors, reset } = useForm({
    id: manpowerRequestData.id, // The ID of the original request (needed for the URL)
    sub_section_id: manpowerRequestData.sub_section_id || '',
    date: manpowerRequestData.date || '',
    time_slots: initialTimeSlots,
    status: manpowerRequestData.status || '',
  });

  // Effect to ensure all shifts are present in time_slots, filling defaults if missing
  useEffect(() => {
  ...
  }, [shifts, manpowerRequestData.time_slots]); // Include manpowerRequestData.time_slots as a dependency
      ...
      if (field === 'requested_amount') {
        ...
        if (amount <= 0) {
          ...
          newTimeSlots[shiftId] = {
            ...newTimeSlots[shiftId],
            start_time: originalShift?.start_time || '', // Reset to original shift time if needed
  ...
  const submit = (e) => {
    ...
    const payloadTimeSlots = {}; // Use an object for time_slots as expected by backend
                               // where keys are shift_id and values are slot data.
    ...
    Object.keys(data.time_slots).forEach(shiftId => {
      ...
      // Only include slots with requested_amount > 0
      if (requestedAmount > 0) {
        payloadTimeSlots[parseInt(shiftId, 10)] = {
          id: slot.id, // Pass the ID if it's an existing record
    ...
    put(route('manpower-requests.update', data.id), {
      sub_section_id: data.sub_section_id,
      date: data.date,
      time_slots: payloadTimeSlots,
    }, {
      onSuccess: () => {
        // Optionally redirect or show success message, then reset
        // For revision, you might want to redirect back to index or the detail page
              ...
              <form onSubmit={submit} className="space-y-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    required
                    disabled // Disable sub_section_id in revision/edit since it's grouped by this
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    disabled // Disable date in revision/edit since it's grouped by this
                  />
                ...
                <div className="space-y-4 mt-4 pt-4 border-t">
                  ...
                  <p className="mb-4 text-gray-600 dark:text-gray-400 text-sm italic">
                    Sesuaikan jumlah man power yang dibutuhkan per shift.
                  </p>
                ...
                <div className="flex justify-end items-center pt-2">
                  <button
                    ...
                    disabled={processing}
                  >
                    {processing ? 'Menyimpan Revisi...' : 'Submit Revisi'}
                  </button>
                </div>
              </form>
            </div>
